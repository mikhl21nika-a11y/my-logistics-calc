...
with bottom_controls:
    ...
    with col2:
        tariff = st.slider("Тариф за 1 км (руб)", min_value=50, max_value=250, value=139, step=1)
...
# Весь HTML-код (теперь мы его безопасно обработаем)
raw_html = f"""
...
# Тот самый хитрый трюк: удаляем ВСЕ переносы строк перед отрисовкой!
